...
namespace PampelGames.Shared.Utility
{
    public static class PGMeshUtility
    {
        ...
        /// <summary>
        ///     Merges the submeshes of the provided mesh, consolidating materials where possible.
        /// </summary>
        public static bool MergeSubMeshes(Mesh mesh, List<Material> materials, out List<Material> mergedMaterials)
        {
            return MergeSubMeshesInternal(mesh, materials, out mergedMaterials);
        }
        ...
        /// <summary>
        ///     EDITOR ONLY
        ///     Generates lightmap UVs (Mesh.uv2) for a mesh.
        /// </summary>
        /// <param name="packMargin">Margin between UV islands (affects bleeding)</param>
        /// <param name="hardAngle">Angle above which edges are considered hard (splits UVs)</param>
        /// <param name="angleError">Max angle error for unwrapping (smaller = tighter UVs, higher texel density)</param>
        /// <param name="areaError">Max area error (smaller = more uniform UVs)</param>
        public static void GenerateLightmapUV(Mesh mesh, float packMargin = 2f, float hardAngle = 88f, float angleError = 8f, float areaError = 15f)
        {
#if UNITY_EDITOR
            UnwrapParam.SetDefaults(out var unwrap);
            unwrap.angleError = angleError;
            unwrap.areaError = areaError;
            unwrap.hardAngle = hardAngle;
            unwrap.packMargin = packMargin * 0.01f;
            Unwrapping.GenerateSecondaryUVSet(mesh, unwrap);
#endif
        }


        /* Pivot (Axis) Orientation ************************************************************************************************************/

        /// <summary>
        ///     Positions the pivot of a transform by modifying the mesh data.
        /// </summary>
        /// <param name="transform">Transform to re-position (Mesh Filter).</param>
        /// <param name="position">The new pivot position in world space.</param>
        public static void SetPivotPosition(Mesh mesh, Transform transform, Vector3 position)
        {
            var vertices = mesh.vertices;
            var localOffset = transform.InverseTransformPoint(position) - transform.InverseTransformPoint(transform.position);
            for (var i = 0; i < vertices.Length; i++) vertices[i] -= localOffset;
        ...
        /// <summary>
        ///     Rotates the pivot of a transform by modifying the mesh data.
        /// </summary>
        /// <param name="transform">Transform to re-orient (Mesh Renderer).</param>
        /// <param name="forward">The new pivot z-axis in world space.</param>
        /// <param name="up">The new pivot y-axis in world space.</param>
        public static void SetPivotRotation(Mesh mesh, Transform transform, Vector3 forward, Vector3 up)
        {
            var vertices = mesh.vertices;
            ...
            var invDelta = Quaternion.Inverse(newRotation) * oldRotation;

            for (var i = 0; i < vertices.Length; i++) vertices[i] = invDelta * vertices[i];
            if (normals is {Length: > 0})
                for (var i = 0; i < normals.Length; i++)
                    normals[i] = invDelta * normals[i];
            if (tangents is {Length: > 0})
                for (var i = 0; i < tangents.Length; i++)
                {
                    var xyz = invDelta * new Vector3(tangents[i].x, tangents[i].y, tangents[i].z);
                    tangents[i] = new Vector4(xyz.x, xyz.y, xyz.z, tangents[i].w);
                }
        ...
        public static void PGRotateCombine(this ref CombineInstance combineInstance, Vector3 pivot, Vector3 eulerRotation)
        {
            ...
            combineInstance.PGRotateCombine(pivot, Quaternion.Euler(eulerRotation));
        }
        ...
        private static bool MergeSubMeshesInternal(Mesh mesh, List<Material> materials, out List<Material> mergedMaterials)
        {
            mergedMaterials = new List<Material>();
            if (!mesh || materials.Count == 0) return false;

            var materialToTriangles = new Dictionary<Material, List<int>>();

            for (var sub = 0; sub < mesh.subMeshCount; sub++)
            {
                var mat = materials[Mathf.Clamp(sub, 0, materials.Count - 1)];

                if (!materialToTriangles.TryGetValue(mat, out var tris))
                {
                    tris = new List<int>();
                    materialToTriangles[mat] = tris;
                }

                tris.AddRange(mesh.GetTriangles(sub));
            }
