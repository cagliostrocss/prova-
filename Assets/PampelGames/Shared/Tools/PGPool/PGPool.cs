...
namespace PampelGames.Shared.Tools
{
    ...
    public static class PGPool
    {
        ...
        public static void Preload(GameObject prefab, int loadAmount, bool dontDestroyOnLoad = true, bool hidePooled = true)
        {
            var loadedObjects = new List<GameObject>();
            for (int i = 0; i < loadAmount; i++) loadedObjects.Add(Get(prefab, dontDestroyOnLoad, hidePooled));
            for (int i = 0; i < loadedObjects.Count; i++) Release(loadedObjects[i]);
        }
        ...
        /// <param name="dontDestroyOnLoad">Do not destroy released GameObjects when a new scene is loaded.</param>
        /// <param name="hidePooled">Sets HideFlags.HideInHierarchy to released GameObjects.</param>
        /// <returns>The Object spawned into the scene.</returns>
        public static GameObject Get(GameObject prefab, bool dontDestroyOnLoad = true, bool hidePooled = true)
        {
            if (!poolDictionary.TryGetValue(prefab.GetInstanceID(), out var poolDescr))
            {
                var pool = new ObjectPool<GameObject>(
                    () => CreateSetup(prefab),
                    GetSetup,
                    obj => ReleaseSetup(obj, dontDestroyOnLoad, hidePooled),
                    DestroySetup);
        ...
        private static void ReleaseSetup(GameObject obj, bool dontDestroyOnLoad, bool hidePooled)
        {
            obj.transform.SetParent(null);
            obj.SetActive(false);
            if (dontDestroyOnLoad) Object.DontDestroyOnLoad(obj);
#if UNITY_EDITOR
            obj.hideFlags = hidePooled ? HideFlags.HideInHierarchy : HideFlags.None;
#endif
        }
