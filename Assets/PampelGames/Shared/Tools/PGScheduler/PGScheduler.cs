...
namespace PampelGames.Shared.Tools
{
    public static class PGScheduler
    {
        ...
        public static PGSchedulerDescr ScheduleFrames(MonoBehaviour mono, int delayFrames, Action action, PGEnums.UpdateMode updateMode = PGEnums.UpdateMode.FixedUpdate)
        {
            if (updateMode == PGEnums.UpdateMode.FixedUpdate) delayFrames++; // Need to add one more frame to get specified frame count
            var scheduler = SetupScheduler(updateMode, delayFrames, true);
            scheduler.internalEvents.onComplete = action;
            var newCoroutine = mono.StartCoroutine(PGSchedulerUpdate._SchedulerUpdate(scheduler));
            scheduler._coroutine = newCoroutine;
            return scheduler;
